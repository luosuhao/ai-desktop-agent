---
description: Native PPTX fill workflow — run only when the user uploads a PowerPoint deck, supplies separate new material, and explicitly asks to clone/fill/replace content in that uploaded deck without SVG conversion
---

# Native PPTX Fill (`template-fill`) Workflow

> Run only when all three conditions are explicit: the user provides a native `.pptx` in the current task, provides separate new material, and asks to clone/fill/replace content in that uploaded PPTX while preserving its native design. A source page may be selected, reordered, omitted, or reused for several output slides.
...
1. **User-provided native PPTX** — the `.pptx` is uploaded/provided by the user in this task, not discovered inside the Skill.
2. **Separate new material** — a document, text, URL-derived source, or clear new topic brief exists independently of the PPTX.
3. **Explicit native-fill intent** — the user refers to the uploaded PPTX and asks to clone its pages, fill its slots, replace its copy/data, or preserve its native design while inserting the new material.

Recognize explicit requests such as:

| Pattern | Example |
|---|---|
| Uploaded PPTX + native fill | "使用我上传的 PPTX 作为原生模板，把这份文档填进去" |
| Uploaded PPTX + page cloning | "克隆这个 PPTX 中合适的页面并替换内容" |
| Uploaded PPTX + selective reuse | "保留这个 PPTX 的原生设计，只选择适合的页面填充新材料" |
| Uploaded PPTX + copy/data replacement | "直接替换我上传 PPTX 里的文案、表格和图表数据" |

### Hard non-triggers

Do **not** run this workflow in any of these cases:

| Input / intent | Correct route |
|---|---|
| Document-only input + "使用模板" / "用模板生成" / "按模板生成" | Main pipeline fixed deck: `${SKILL_DIR}/templates/decks/guotai_fund_standard/` |
| Document-only input with no template instruction | Main pipeline free design + `${SKILL_DIR}/templates/页面素材.pptx` design-reference intake |
| Skill-internal PPTX or registered deck package | Never counts as user-provided PPTX; stay in the main pipeline |
| Uploaded PPTX used only as content/reference and the story may be rebuilt | Main pipeline |
| Uploaded PPTX whose page count/order/wording must remain 1:1 | `beautify` |
| Uploaded PPTX to be harvested as a reusable package | `create-template` |

If a user-uploaded PPTX and separate material are both present but the intended role is unclear, ask one question before routing: "这个 PPTX 是作为内容参考重新组织、作为原生页面模板克隆填充，还是保持页数/顺序/文案不变只做美化？" Do not infer native-fill intent from the word "模板" alone.
...
|---|---:|---|
| Source PPTX | Yes | Native deck explicitly uploaded/provided by the user in the current task; Skill-internal resources do not qualify |
| Content material | Yes | Separate user text, Markdown, document, URL-derived source, or a clear topic brief |
| Native-fill intent | Yes | Explicit request to clone/fill/replace content in the uploaded PPTX while preserving its native design |
| Target output details | Optional | Audience, page count, tone, must-keep pages, must-drop pages |
