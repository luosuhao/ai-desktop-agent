...
🚧 **GATE — free-design DNA evidence**: when `<project>/analysis/design_reference/` exists, read `dna_summary.md` before constructing any recommendation. If the folder exists but `dna_summary.md` does not, return to SKILL.md Step 3 and complete the intake/read gate; do not reconstruct the palette from industry intuition. This gate affects visual identity only: the user's Markdown remains the content contract, and storyline/page count/order/composition remain free.
...
**Hard precedence**: explicit user color choice > loaded Step 3 template > free-design DNA baseline > industry/content default.

- If the user specified colors (HEX, brand colors, or directives such as "use blue as primary"), or a template was loaded at Step 3 via an explicit/fixed deck path (`<project_path>/templates/design_spec.md`), lock those directly and skip the normal recommendation table. Do not adjust them to fit an industry default.
- If free design produced `<project>/analysis/design_reference/dna_summary.md`, do **not** skip user choice: still author ≥3 color candidates, but candidate A MUST be the source-heritage palette copied from the summary, MUST be pre-selected with `color.selected: 0`, and its note MUST identify the built-in reference PPTX as the source. Candidates B/C may be content-aware alternatives. Finance navy, tech blue, or another industry default may appear only as B/C; it may not displace A because of topic fit.
- Only when none of the signals above exists may the industry/content table choose candidate A.

For the currently bundled reference, the expected source-heritage role mapping is background `#FFFFFF`, secondary background `#F4EDE5`, primary `#9E0116`, accent `#BF9D5A`, secondary accent `#5D070C`, and near-black body text `#1A1A1A`. Read the live summary as truth if the bundled PPTX is later replaced.

**Confirmation propagation**: a plain confirmation accepts pre-selected candidate A. If the user explicitly selects B/C or supplies a custom palette, that choice wins. Write the confirmed palette identically into `design_spec.md §III Visual Theme` and `spec_lock.md colors`; source heritage is the default, never an override of an explicit user choice.
