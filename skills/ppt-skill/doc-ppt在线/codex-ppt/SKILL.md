...
metadata:
  openclaw:
    ...
    envVars:
      ...
      - name: CODEX_PPT_NATIVE_RUNTIME
        required: false
        description: Native editable tail runtime, defaults to pptxgenjs for Route B.
...
This skill creates image-based PowerPoint decks from source material. Each slide is a complete 16:9 generated image. Final images are assembled into `.pptx` with `scripts/assemble_ppt.py`. When the user explicitly asks for editable output, finish the image-based deck first, then use the agreed editable tail route in `docs/editable-postprocess.md`.

Use this when the user wants a visually unified presentation and accepts full-slide image pages, or when the agreed workflow is an image deck followed by one of the editable tail routes. Do not present the first-stage image-based PPTX as fully editable.
...
- During final slide image generation, local drawing, Pillow, SVG, HTML/CSS/canvas screenshots, python-pptx/PptxGenJS layouts, and manual overlays are failure modes, not fallbacks. The native editable tail route is the only exception, and it starts only after the image-based deck has been completed.
...
- The canonical six-color palette in `assets/palettes/corporate-six.json` is a hard deck constraint for generated slide design and visual assets unless the user explicitly changes it.
...
12. Optionally produce an editable PPTX through the agreed tail route.
    - Do this only when the user asks for editable output or explicitly approves the two-stage route.
    - Before starting, read `docs/editable-postprocess.md`.
    - Start only after the image-based PPTX and final `origin_image/slide_XX.png` files exist. Do not feed draft images.
    - Default to Route A (`image_to_editable_ppt`) when the user asks for editable output but does not choose a route. Use Route B (`native editable rebuild` with PptxGenJS by default) only when the user explicitly chooses it or clearly prioritizes text-stable native rebuilding over visual reconstruction.
    - Keep editable tail state under the deck project directory, separate from slide-generation state.
...
- `docs/editable-postprocess.md`: optional embedded editable reconstruction after the image-based deck is assembled.
- `scripts/build_native_pptx_dynamic.mjs`: preferred Route B dynamic native editable PptxGenJS builder.
- `native_templates/templates.json` and `scripts/build_native_pptx.mjs`: fixed Route B template fallback library and PptxGenJS builder.
- `prompts/slide-worker.md`: slide subagent handoff template.
- `editable/SKILL.md`: embedded image-to-editable module used only for editable postprocessing.
