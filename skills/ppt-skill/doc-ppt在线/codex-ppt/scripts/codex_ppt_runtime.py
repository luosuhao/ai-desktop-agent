...
def _config(args: argparse.Namespace) -> int:
    ...
    if args.provider is not None:
        values["CODEX_PPT_IMAGE_PROVIDER"] = args.provider.strip()
    if args.dashscope_api_key:
        values["DASHSCOPE_API_KEY"] = args.dashscope_api_key
    ...
    if args.clear_provider:
        values.pop("CODEX_PPT_IMAGE_PROVIDER", None)
...
def _uses_qwen_dashscope(provider: str, model: str) -> bool:
    if provider.strip().lower() in SILICONFLOW_PROVIDERS:
        return False
    return provider.strip().lower() in QWEN_PROVIDERS or QWEN_IMAGE_MODEL_PREFIX in model.lower()


def _uses_siliconflow(provider: str, base_url: str) -> bool:
    if provider.strip().lower() in SILICONFLOW_PROVIDERS:
        return True
    hostname = urllib.parse.urlparse(base_url).hostname or ""
...
def _doctor(args: argparse.Namespace) -> int:
    ...
    provider = values.get("CODEX_PPT_IMAGE_PROVIDER", "")
    ...
    siliconflow_selected = _uses_siliconflow(provider, base_url)
    qwen_selected = _uses_qwen_dashscope(provider, model)

    print(f"OPENAI_API_KEY={'set (' + _mask_secret(api_key) + ')' if api_key else '<unset>'}")
    print(f"OPENAI_BASE_URL={base_url}")
    print(f"CODEX_PPT_IMAGE_MODEL={model}")
    print(f"CODEX_PPT_IMAGE_PROVIDER={provider or '<unset>'}")
    print(
        f"DASHSCOPE_API_KEY={'set (' + _mask_secret(dashscope_api_key) + ')' if dashscope_api_key else '<unset>'}"
    )
    print(f"DASHSCOPE_BASE_URL={dashscope_base_url}")

    if siliconflow_selected:
        if QWEN_IMAGE_MODEL_PREFIX not in model.lower():
            print("model check: warning, SiliconFlow Qwen should use a model containing 'qwen-image'")
            ok = False
        else:
            print("model check: ok")
    elif qwen_selected:
        if QWEN_IMAGE_MODEL_PREFIX not in model.lower():
            print("model check: warning, qwen-dashscope should use a model containing 'qwen-image'")
            ok = False
        else:
            print("model check: ok")
    ...
    if args.check_api:
        if siliconflow_selected:
            if api_key:
                print("api check: SiliconFlow configured (network probe not performed by doctor)")
            else:
                print("api check: skipped, OPENAI_API_KEY is unset")
                ok = False
        elif qwen_selected:
            if dashscope_api_key:
                print("api check: qwen-dashscope configured (network probe not performed by doctor)")
            else:
                print("api check: skipped, DASHSCOPE_API_KEY is unset")
                ok = False
        elif not api_key:
            print("api check: skipped, OPENAI_API_KEY is unset")
            ok = False
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    config.add_argument("--provider", help="Image provider selector, for example qwen-dashscope.")
    config.add_argument("--clear-provider", action="store_true")
    config.add_argument("--dashscope-api-key")
    config.add_argument("--dashscope-base-url")
    config.add_argument("--clear-dashscope-base-url", action="store_true")
