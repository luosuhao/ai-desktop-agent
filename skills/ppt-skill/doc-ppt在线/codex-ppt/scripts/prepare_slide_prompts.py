...
def _palette_prompt_text(palette: Optional[Dict[str, Any]]) -> str:
    if not palette:
        return ""
    prompt_text = str(palette.get("prompt_text") or "").strip()
    if prompt_text:
        return prompt_text
...
def _build_prompt(
    *,
    ...
    prompt_parts = [
        ...
        _format_block("Design System Color Rules", _palette_prompt_text(palette)),
    ]
    ...
    palette_constraint = _palette_prompt_text(palette)
    if palette_constraint:
        universal_constraints.append(palette_constraint)
    universal_constraints.extend(
        [
    ...
    prompt_parts.append(
        ...
        + "\n".join(f"- {item}" for item in universal_constraints)
        + "\n"
    )
...
def _write_template(path: Path) -> None:
    ...
    template = {
        ...
        "style": {
            "name": "白皮书技术路线信息图 × 学术智库报告",
            "visual_direction": (
                "research white-paper technical-route infographic; pure white full-slide background; "
                "modular multi-panel cards, numbered process pipelines, evidence-chain boxes, "
                "source table snapshots, compact framework diagrams, entity-indicator-event relationship "
                "maps, risk matrices, governance loops, stakeholder responsibility matrices, layered "
                "strategy tables, timelines, and method architecture diagrams; not premium business keynote, "
                "launch-event, dark tech, glossy pitch style, or oversized hero-slogan style"
            ),
            "color_palette": (
                "pure white full-slide background; foreground components use the corporate-six "
                "palette from assets/palettes/corporate-six.json"
            ),
            "typography": "clear Chinese research-report typography, restrained hierarchy, no slogan-like hero typography",
            "texture_and_finish": "clean white-paper/report finish; avoid glossy gradients, dark tech texture, and decorative lighting",
        },
        "palette_contract": {
            "path": str(DEFAULT_PALETTE_PATH),
        ...
        "slides": [
            {
                ...
                "layout": {"composition": "title and thesis area with a compact framework or evidence motif"},
                "visual_elements": {"main_visual": "topic-specific framework diagram or evidence-chain motif"},
...
def main() -> int:
    ...
    parser.add_argument(
        "--palette",
        help=f"Palette contract JSON. Defaults to {DEFAULT_PALETTE_PATH}.",
    )
    ...
    palette_path = Path(args.palette) if args.palette else DEFAULT_PALETTE_PATH
    palette = _load_json_file(palette_path, label="Palette")
    ...
    for fallback, slide, number in numbered_slides:
        ...
        job = {
            ...
            "palette_contract": {
                ...
                "hard_requirement": bool(palette.get("hard_requirement", True)),
                "prompt_text": _palette_prompt_text(palette),
            },
            "generation_contract": {
