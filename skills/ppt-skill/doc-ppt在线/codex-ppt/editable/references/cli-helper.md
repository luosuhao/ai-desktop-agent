...
`editppt image` automatically chooses the image backend: Codex OAuth first, then OpenAI-compatible API credentials from `~/.editppt/config.yaml` or environment variables if OAuth is unavailable. When the provider is `qwen-dashscope` or the model contains `qwen-image`, it uses DashScope Qwen Image directly for both generation and edit calls. When the provider is `siliconflow`, it uses SiliconFlow's `/v1/images/generations` API; edit calls automatically switch `Qwen/Qwen-Image` to `Qwen/Qwen-Image-Edit-2509`.
...
editppt config --provider siliconflow --api-key "<siliconflow-key>" --base-url https://api.siliconflow.cn/v1 --model Qwen/Qwen-Image
editppt config --provider qwen-dashscope --dashscope-api-key "<dashscope-key>" --model qwen-image-2.0-pro
```
...
These commands call the selected image backend: Codex OAuth first, then a configured OpenAI-compatible API fallback. DashScope Qwen Image is selected by `--model qwen-image-2.0-pro` or `IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER=qwen-dashscope` and uses `DASHSCOPE_API_KEY` instead of `OPENAI_API_KEY`. SiliconFlow Qwen Image is selected by `IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER=siliconflow` and uses `OPENAI_API_KEY` plus `OPENAI_BASE_URL=https://api.siliconflow.cn/v1`. In a network-restricted runtime, request approval before the call and state that only task-local prompts plus required page images/masks/references are uploaded for the current conversion.
