...
QWEN_PROVIDERS = {"qwen", "qwen-image", "qwen-dashscope", "dashscope"}
SILICONFLOW_PROVIDERS = {"siliconflow", "silicon-flow"}
ENV_FIELDS = (
    "OPENAI_API_KEY",
    "OPENAI_BASE_URL",
    "IMAGE_TO_EDITABLE_PPT_IMAGE_MODEL",
    "IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER",
    "DASHSCOPE_API_KEY",
    "DASHSCOPE_BASE_URL",
    "PADDLE_OCR_TOKEN",
)
...
CODEX_PPT_ENV_MAP = {
    ...
    "CODEX_PPT_IMAGE_PROVIDER": "IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER",
...
def uses_qwen_dashscope(provider, model):
    if str(provider or "").strip().lower() in SILICONFLOW_PROVIDERS:
        return False
    return str(provider or "").strip().lower() in QWEN_PROVIDERS or QWEN_IMAGE_MODEL_PREFIX in str(model or "").lower()


def uses_siliconflow(provider, base_url):
    if str(provider or "").strip().lower() in SILICONFLOW_PROVIDERS:
        return True
    return "siliconflow" in str(base_url or "").lower()
...
def config(args):
    ...
    if args.provider is not None:
        values["IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER"] = args.provider.strip()
    if args.clear_provider:
        values.pop("IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER", None)
    if args.dashscope_api_key:
        values["DASHSCOPE_API_KEY"] = args.dashscope_api_key.strip()
    if args.dashscope_base_url is not None:
        values["DASHSCOPE_BASE_URL"] = args.dashscope_base_url.strip()
    if args.clear_dashscope_base_url:
        values.pop("DASHSCOPE_BASE_URL", None)
    ...
    for key in ENV_FIELDS:
        ...
        if key in ("OPENAI_API_KEY", "DASHSCOPE_API_KEY", "PADDLE_OCR_TOKEN"):
...
def collect_status(check_api=False):
    ...
    model = values.get("IMAGE_TO_EDITABLE_PPT_IMAGE_MODEL", DEFAULT_IMAGE_MODEL)
    provider = values.get("IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER", "")
    base_url = values.get("OPENAI_BASE_URL", "https://api.openai.com/v1")
    siliconflow_selected = uses_siliconflow(provider, base_url)
    qwen_selected = uses_qwen_dashscope(provider, model)
    api_ready = bool(api_key)
    codex_ready = codex_oauth_ready()
    dashscope_ready = bool(dashscope_api_key)
    image_backend_ready = api_ready if siliconflow_selected else (dashscope_ready if qwen_selected else (codex_ready or api_ready))
    image_backend_selection = (
        "siliconflow"
        if siliconflow_selected
        else "qwen-dashscope"
        if qwen_selected
        else ("codex-oauth" if codex_ready else ("openai-compatible-api" if api_ready else "missing"))
    )
    ...
    return {
        ...
        "api_fallback": {
            ...
            "IMAGE_TO_EDITABLE_PPT_IMAGE_MODEL": model,
            "IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER": provider or "unset",
            "siliconflow_selected": siliconflow_selected,
            "siliconflow_ready": api_ready,
            "DASHSCOPE_API_KEY": "set" if dashscope_api_key else "unset",
            "DASHSCOPE_BASE_URL": values.get("DASHSCOPE_BASE_URL", DEFAULT_DASHSCOPE_BASE_URL),
            "qwen_dashscope_selected": qwen_selected,
            "qwen_dashscope_ready": dashscope_ready,
        ...
        "image_backend": {
            ...
            "selection": image_backend_selection,
        },
        "text_hints": {
        ...
        "next": "no action needed" if ok else (
            "run `editppt config --provider siliconflow --api-key <key> --base-url https://api.siliconflow.cn/v1 --model Qwen/Qwen-Image`"
            if check_api and siliconflow_selected and not api_ready
            else "run `editppt config --provider qwen-dashscope --dashscope-api-key <key> --model qwen-image-2.0-pro`"
            if check_api and qwen_selected and not dashscope_ready
            else (
                "run `codex login` or `editppt config --api-key <key>`"
                if check_api and not image_backend_ready
...
def doctor(args):
    ...
    print(f"IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER={api['IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER']}")
    print(f"DASHSCOPE_API_KEY={api['DASHSCOPE_API_KEY']}")
    print(f"DASHSCOPE_BASE_URL={api['DASHSCOPE_BASE_URL']}")
    ...
    if args.check_api:
        if image_backend["ready"]:
            ...
            if api["siliconflow_selected"]:
                print("image backend check: siliconflow selected but OPENAI_API_KEY is missing")
            elif api["qwen_dashscope_selected"]:
                print("image backend check: qwen-dashscope selected but DASHSCOPE_API_KEY is missing")
            else:
...
def main():
    ...
    cfg.add_argument("--provider", help="Image provider selector, for example qwen-dashscope.")
    cfg.add_argument("--clear-provider", action="store_true", help="Remove IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER from the config file.")
    cfg.add_argument("--dashscope-api-key", help="DashScope API key for Qwen Image.")
    cfg.add_argument("--dashscope-base-url", help="DashScope or Model Studio /api/v1 base URL.")
    cfg.add_argument("--clear-dashscope-base-url", action="store_true", help="Remove DASHSCOPE_BASE_URL from the config file.")
