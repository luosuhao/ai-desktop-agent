...
CODEX_PPT_ROOT = Path(__file__).resolve().parents[4]
CODEX_PPT_SCRIPTS = CODEX_PPT_ROOT / "scripts"
DEFAULT_PALETTE_PATH = CODEX_PPT_ROOT / "assets" / "palettes" / "corporate-six.json"
if str(CODEX_PPT_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(CODEX_PPT_SCRIPTS))

from image_providers.qwen_dashscope import (  # noqa: E402
    QwenDashScopeImageProvider,
    normalize_qwen_size,
    preview_endpoint as qwen_preview_endpoint,
)
from image_providers.siliconflow import (  # noqa: E402
...
Backend selection:
  ...
  SiliconFlow: use IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER=siliconflow,
  OPENAI_BASE_URL=https://api.siliconflow.cn/v1, and OPENAI_API_KEY.
  Qwen Image: use IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER=qwen-dashscope or a
  qwen-image model plus DASHSCOPE_API_KEY.
...
Setup:
  ...
  editppt config --provider siliconflow --api-key "your-siliconflow-api-key" --base-url https://api.siliconflow.cn/v1 --model Qwen/Qwen-Image
  editppt config --provider qwen-dashscope --dashscope-api-key "your-dashscope-api-key" --model qwen-image-2.0-pro
...
def _image_provider_name() -> Optional[str]:
    return os.getenv("IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER") or os.getenv("CODEX_PPT_IMAGE_PROVIDER") or None
...
def _ensure_api_key(dry_run: bool, model: str) -> None:
    if _uses_siliconflow(model):
        ...
        if dry_run:
            ...
            return
        _die(
            "OPENAI_API_KEY is required for SiliconFlow image generation/editing.\n"
            "Run: editppt config --provider siliconflow --api-key <key> "
    ...
    if _uses_qwen_dashscope(model):
        ...
        if dry_run:
            ...
            return
        _die(
            "DASHSCOPE_API_KEY is required for qwen-dashscope image generation/editing.\n"
            "Set DASHSCOPE_API_KEY and use --model qwen-image-2.0-pro. "
...
def _is_siliconflow_base_url(base_url: Optional[str]) -> bool:
    if not base_url:
        return False
    parsed = re.match(r"https?://([^/:]+)", base_url.strip(), re.I)
    return bool(parsed and "siliconflow" in parsed.group(1).lower())
...
def _run_qwen_dashscope_image(
    *,
    ...
    if not _uses_qwen_dashscope(args.model):
        return False
    payload = {
        "model": args.model,
        "prompt": prompt,
        ...
        _print_request(
            {
                ...
                "endpoint": qwen_preview_endpoint(_dashscope_base_url()),
                ...
                "size": args.size,
                "effective_size": normalize_qwen_size(args.size),
            }
        )
        return True

    provider = QwenDashScopeImageProvider(
        api_key=_dashscope_api_key(),
        base_url=_dashscope_base_url(),
    )
    images = provider.edit(payload, image_paths, mask_path) if image_paths else provider.generate(payload)
    _decode_and_write(images, output_paths, force=args.force)
    return True


def _run_siliconflow_image(
    *,
    prompt: str,
    image_paths: List[Path],
    mask_path: Optional[Path],
    ...
    if not _uses_siliconflow(args.model):
        return False
    operation = "edit" if image_paths else "generate"
    payload = {
        "model": args.model,
    ...
    if args.dry_run:
        _print_request(
            {
                "backend": "siliconflow",
                "endpoint": siliconflow_preview_endpoint(_api_base_url()),
                "operation": operation,
                "outputs": [str(p) for p in output_paths],
                "model": siliconflow_model_for_operation(args.model, operation),
                "prompt": prompt,
                "input_images": [str(p) for p in image_paths],
                "mask": str(mask_path) if mask_path else None,
                "size": args.size,
                "effective_size": siliconflow_effective_size(payload, operation),
            }
        )
        return True
...
def _run_codex_image(
    *,
    ...
    if _uses_siliconflow(args.model) or _uses_qwen_dashscope(args.model):
        return False
...
def _generate(args: argparse.Namespace) -> None:
    prompt = _apply_palette_constraint(_read_prompt(args.prompt, args.prompt_file))
...
""",
    ...
    if _uses_siliconflow(args.model) or _uses_qwen_dashscope(args.model) or not _codex_available():
