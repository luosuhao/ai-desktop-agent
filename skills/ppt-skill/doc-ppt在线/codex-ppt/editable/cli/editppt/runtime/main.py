...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if args.clear_provider:
        argv.append("--clear-provider")
    if args.dashscope_api_key:
        argv.extend(["--dashscope-api-key", args.dashscope_api_key])
    if args.dashscope_base_url:
        argv.extend(["--dashscope-base-url", args.dashscope_base_url])
    if args.clear_dashscope_base_url:
        argv.append("--clear-dashscope-base-url")
...
""",
    ...
    config.add_argument("--clear-provider", action="store_true", help="Remove IMAGE_TO_EDITABLE_PPT_IMAGE_PROVIDER from the config file.")
    config.add_argument("--dashscope-api-key", metavar="KEY", help="DashScope API key for Qwen Image.")
    config.add_argument("--dashscope-base-url", metavar="URL", help="DashScope or Model Studio /api/v1 base URL.")
    config.add_argument("--clear-dashscope-base-url", action="store_true", help="Remove DASHSCOPE_BASE_URL from the config file.")
...
SiliconFlow, or Qwen DashScope when configured. Use process-sheet for deterministic
...
""",
  ...
  and IMAGE_TO_EDITABLE_PPT_IMAGE_MODEL. SiliconFlow uses OPENAI_API_KEY,
  OPENAI_BASE_URL=https://api.siliconflow.cn/v1, and provider/model settings.
  Qwen DashScope uses DASHSCOPE_API_KEY plus provider/model settings.
