...
For academic papers, policy studies, industry research reports, financial compliance documents, governance strategy documents, or topics involving ethics/risk/regulation, recommend `白皮书技术路线信息图 × 学术智库报告` as the first option unless the user asks for a different tone. This style means:

- Pure white full-slide background.
- Restrained research-report layout, not executive keynote or premium business pitch style.
- Medium-high information density with stable margins, clear headings, and report-like hierarchy.
- Prefer modular multi-panel cards, numbered process pipelines, evidence-chain boxes, source table snapshots, compact framework diagrams, entity-indicator-event relationship maps, risk matrices, governance loops, stakeholder responsibility matrices, layered strategy tables, timelines, and method architecture diagrams.
- Avoid glossy business presentation style, launch-event style, dark tech style, large decorative gradients, oversized hero slogans, and ornamental illustration-first pages.
- Use the six-color palette only for foreground components, labels, nodes, arrows, table headers, and local emphasis.
...
The default palette is fixed in `assets/palettes/corporate-six.json` and mirrored by `assets/palettes/palette.png`. Treat it as a hard requirement when planning or confirming a visual style:

- `#9E0116` primary red
- `#5D070C` deep burgundy
- `#706F71` charcoal gray
- `#A7A7A7` medium gray
- `#BF9D5A` muted gold
- `#D8C49B` sand beige

Use a pure white background as the full slide canvas. Do not place a white slide card on top of a dark, red, burgundy, blurred, or decorative outer background. Use these six colors for foreground components: generated icons, shapes, callouts, charts, section markers, dividers, labels, and non-photographic visual assets. Use primary red and deep burgundy only for accents, headings, key nodes, and emphasis, not as a full-slide background or large dominant surface. White, black, transparency, and subtle tints/shades of the six colors are allowed for readability, shadows, soft panels, and edge separation. Do not introduce blue, green, purple, orange, cyan, or other new accent hues unless preserving a strict user-supplied source image. The palette is an internal design rule only: never render color cards, color-code text, color names, palette labels, filler microtext, fake UI chrome, or decorative menu icons as slide content.
