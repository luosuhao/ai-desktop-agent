...
The local API/CLI fallback supports OpenAI-compatible image APIs, AtlasCloud, DashScope Qwen Image, and SiliconFlow Qwen Image. Qwen Image is an optional provider choice, not a required backend. When `CODEX_PPT_IMAGE_PROVIDER=qwen-dashscope` or the model name contains `qwen-image`, the fallback calls DashScope directly instead of the OpenAI Images API shape. When `CODEX_PPT_IMAGE_PROVIDER=siliconflow`, the fallback calls SiliconFlow's `/v1/images/generations` request shape.
...
- Do not force Qwen Image. Use Qwen Image only when the user explicitly selects it, the runtime has already been configured for a Qwen provider and the user approves using that API fallback, or another backend is unavailable and Qwen is the agreed fallback.
...
- Ask for `OPENAI_API_KEY` or `DASHSCOPE_API_KEY` configuration only after you have intentionally selected CLI/API fallback and that fallback reports missing config, after authentication/base URL/model errors, or when the user explicitly wants to change API settings. Do not mention missing API keys while the Codex built-in image tool is available. Configure OpenAI-compatible values with `scripts/codex_ppt_runtime.py config --api-key`; configure DashScope Qwen with `scripts/codex_ppt_runtime.py config --provider qwen-dashscope --dashscope-api-key <key> --model qwen-image-2.0-pro`; configure SiliconFlow Qwen with `scripts/codex_ppt_runtime.py config --provider siliconflow --api-key <key> --base-url https://api.siliconflow.cn/v1 --model Qwen/Qwen-Image`.
